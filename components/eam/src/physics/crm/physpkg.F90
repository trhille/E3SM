...
!$OMP PARALLEL DO PRIVATE (C, beg_count, phys_buffer_chunk, end_count, chunk_cost)
  ...
  do c=begchunk, endchunk
    ...
    if (use_ECPP) then
      ! crm_ecpp_output_chunk => crm_ecpp_output(c)
      ! call crm_ecpp_output_chunk%initialize(phys_state(c)%ncol,pver)
      ! call crm_ecpp_output(c)%initialize(phys_state(c)%ncol,pver)
